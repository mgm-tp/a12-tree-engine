...
import { EDIT_ENGINE_EVENT } from "../helpers.js";
...
function* handle(action: Action<TreeEngineActions.EventPayload<Action<Events.EventButtonClickedPayload>>>) {
	const { engineAction, activityId } = action.payload;
	if (engineAction.payload.button.event === EDIT_ENGINE_EVENT) {
		const notificationAction = NotificationActions.add({
			activityId,
			duration: 5000,
			severity: "info",
			title: { key: SHOWCASE_RESOURCE_KEYS.showcase.button.eventEngineEdit.title },
			message: { key: SHOWCASE_RESOURCE_KEYS.showcase.button.eventEngineEdit.message }
		});
