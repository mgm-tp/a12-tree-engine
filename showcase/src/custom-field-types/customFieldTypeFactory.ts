...
export class CustomFieldTypeFactory implements ICustomFieldTypeFactory {
	supportedTypeNames: Set<string>;

	constructor() {
		this.supportedTypeNames = new Set<string>(["Email"]);
	}
	...
	createCustomFieldType(customFieldTypeName: string): ICustomFieldType {
		if (customFieldTypeName === "Email") {
			...
			throw new Error(`Could not identify the custom field type: ${customFieldTypeName}`);
		}
	}
}
