...
The default action when a row is clicked in Tree Engine is to view/edit the selected node. This default behavior can be changed via the `rowActivation` property on a node type in the Tree Model. See <<#row-activation, Row Activation>> for the full list of available types and configuration options.
