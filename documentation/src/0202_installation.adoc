...
Tree Engine is provided as an npm package published as ES Modules (ESM). Run the following command to install Tree Engine:
...
The registration process of Tree Engine is quite simple, it is only required to register two Tree Engine modules (link:./assets/generated/typedoc/modules/extensions_client.TreeEngineFactories.html[TreeEngineFactories] and link:./assets/generated/typedoc/modules/extensions_server-connector.TreeEngineServerConnectorFactories.html[TreeEngineServerConnectorFactories]).

[source,typescript,options="nowrap",title="src/appsetup.ts"]
--
import { TreeEngineFactories, TreeEngineServerConnectorFactories } from "@com.mgmtp.a12.treeengine/treeengine-core";
