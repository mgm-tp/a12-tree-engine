 ...
 * @internal
 */
export const onEventButtonClickedRequestMiddleware: Middleware = (api) => (next) => (action) => {
	const result = next(action);
	if (Events.onEventButtonClickedRequest.match(action)) {
		const { button, confirmation } = action.payload;
		const state: TreeEngineState.Dialog.Confirmation.EventButton = {
