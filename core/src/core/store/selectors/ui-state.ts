...
export namespace UIStateSelector {
	...
	const lastMultiSelectionActionReselect = createSelector(
		...
		(multiSelectionActions) => {
			return multiSelectionActions.toReversed().find(Events.onNodeMultiSelectionClicked.match);
		}
	);
	...
	const lastRangeMultiSelectionActionReselect = createSelector(
		[previousMultiSelectionActions()],
		(multiSelectionActions) => {
			return multiSelectionActions.toReversed().find(Events.onNodeRangeSelectionClicked.match);
		}
	);
	 ...
	 * @internal
	 */
	export function isMultiSelectionRowClickActive(): Selector<boolean, TreeEngineState> {
		return (state) => {
			const config = state.models.uiModel.content.configuration.multiSelection;
			const selectionArea =
				config?.selectionArea ?? TreeModel.MultiSelectionConfiguration.SelectionArea.CHECKBOX_AND_ROW;

			if (selectionArea === TreeModel.MultiSelectionConfiguration.SelectionArea.CHECKBOX) {
				return false;
			}

			if (config?.collapseOption === TreeModel.MultiSelectionConfiguration.CollapseOption.NON_COLLAPSIBLE) {
				return overallMultiSelection()(state) !== TreeEngineState.MultiSelectionState.DESELECTED;
			}

			return expandedMultiSelectionPanel()(state);
		};
	}
}
