...
import { DefaultTableComponentRenderers } from "@com.mgmtp.a12.widgets/widgets-core/lib/table/new-api/table.view.js";
...
} from "../../context/tree-engine-context-provider.js";
import { DndUtils } from "../../configuration/dnd/utils.js";
import { type PickStateFromSelectorCreator } from "../../../../store/internal/shared.js";

import { useMultiSelectionConfig } from "../content-box/sub-components/multi-selection/utils.js";
...
export function useEventHandlers(): TreeTableRowEventHandlers<FlattenNodeRow> {
	...
	const expandedMultiSelectionPanelRef = useTreeEngineStateRef(UIStateSelector.expandedMultiSelectionPanel());
	const collapseOption = useMultiSelectionConfig()?.collapseOption;
	const selectionArea =
		useMultiSelectionConfig()?.selectionArea ?? TreeModel.MultiSelectionConfiguration.SelectionArea.CHECKBOX_AND_ROW;
	const rowStylingRef = useTreeEngineContextRef((context) => context.rowStyling);
	const hasSelectedRowRef = useTreeEngineContextRef((context) => context.overallMultiSelection);
	...
	const onClickHandler = React.useCallback(
		(row: FlattenNodeRow) => (event: React.MouseEvent<HTMLElement>) => {
			if (
				...
				expandedMultiSelectionPanelRef.current
			) {
		...
		[collapseOption, expandedMultiSelectionPanelRef, hasSelectedRowRef, onRowClicked, selectRows, selectionArea]
	);
	...
	return React.useCallback(
		({ row }) => {
			...
			return {
				...
				style: rowStyle,
				title: readonly || disabled || busy ? undefined : title,
				...rowStyling?.({ row })
			};
		},
		[
...
function useComponentRenderers(): Partial<Renderers> {
	...
	const dndBodyRowRenderer: Renderers["dndBodyRowRenderer"] = React.useCallback(
		(params) => {
			...
			if (PaginatedRow.isAssignableFrom(params.row)) {
				return <PaginatedBodyRow {...params} row={params.row} />;
			}
			return <DndBodyRow {...params} row={params.row} />;
		},
		[DndBodyRow]
	);
	...
	const bodyRowRenderer: Renderers["bodyRowRenderer"] = React.useCallback(
		(params) => {
			if (RootNodeRow.isAssignableFrom(params.row)) {
				return <VirtualRootRow {...params} row={params.row} />;
			}
			if (PaginatedRow.isAssignableFrom(params.row)) {
				return <PaginatedBodyRow {...params} row={params.row} />;
			}

			return <BodyRow {...params} row={params.row} />;
		},
