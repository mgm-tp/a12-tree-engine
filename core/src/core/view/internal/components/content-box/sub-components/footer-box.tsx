...
export const FooterBox: React.FC<FooterBox.Props> = React.memo(function FooterBox({ ariaLevel }) {
	...
	const majorElements: React.ReactNode[] = React.useMemo(
		() =>
			footerBox.majorElements.map((button) => {
				return <Button key={button.id} element={button} componentKeys={footerBoxButtonKey} />;
			}) ?? [],
		[Button, footerBox.majorElements, footerBoxButtonKey]
	);

	const minorElements: React.ReactNode[] = React.useMemo(
		() =>
			footerBox.minorElements.map((button) => {
				return <Button key={button.id} element={button} componentKeys={footerBoxButtonKey} />;
			}) ?? [],
		[Button, footerBox.minorElements, footerBoxButtonKey]
	);

	return (
		<Footer ariaLevel={ariaLevel}>
			<ButtonGroupContainer responsive leftSlot={minorElements} rightSlot={majorElements} />
		</Footer>
	);
});
