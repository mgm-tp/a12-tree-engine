...
export function useEngineEventActions(isAvailable = true): EngineEventAction[] {
	...
	return React.useMemo(() => {
		...
		const contentBoxButtons: TreeModel.ButtonElement[] = [
			footerBox.minorElements,
			footerBox.majorElements,
			subHeaderBox.minorElements.filter(TreeModel.ButtonElement.isAssignableFrom),
			subHeaderBox.majorElements.filter(TreeModel.ButtonElement.isAssignableFrom)
		].flat();
