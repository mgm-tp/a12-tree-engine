...
import type { DataState, ModelsState } from "../../../store/store.js";
...
export interface DndConfiguration extends Omit<
	TreeTableDragDropOptions<FlattenNodeRow, DragObject, DropResult, HoveredObject>,
	"canDrop"
> {
	acceptType: string;
	...
	return {
		...
		canDrop({ dragItem, hoveredItem }) {
			for (const rule of rules) {
				const result = rule.canDrop({ dragItem, hoveredItem, engineState });
