...
import { Commands, Events } from "../../store/actions.js";
import type { Identifier, TreeEngineState } from "../../store/store.js";
...
const BUILT_IN_EVENT_NAMES = new Set<string>(TreeModel.BUILT_IN_EVENT_NAMES);
...
export function defaultMapDispatchToEventHandlers(dispatch: Dispatch): EventHandlersDispatchMap {
	...
	const onNodeEventButtonClicked: EventHandlersDispatchMap["onNodeEventButtonClicked"] = ({
		nodeIdentifier,
		nodePath,
		button
	}) => {
	...
	const onInsertChildNodeButtonClicked: EventHandlersDispatchMap["onInsertChildNodeButtonClicked"] = ({
		nodeIdentifier,
		nodePath,
		button
	}) => {
		...
		onRowClicked({ nodeModel, nodeIdentifier, nodePath }) {
			...
			switch (rowActivation.type) {
				case "non_interactive":
					return;
				case "event":
					if (BUILT_IN_EVENT_NAMES.has(rowActivation.event)) {
						onNodeEventButtonClicked({
							nodeIdentifier,
							nodePath,
							button: { type: "event", event: rowActivation.event }
						});
					} else {
						dispatch(Events.onRowClicked({ nodeIdentifier, nodePath, event: rowActivation.event }));
					}
					return;
				case "insert":
					onInsertChildNodeButtonClicked({
						...
						button: {
							type: "insert",
							position: rowActivation.position ?? TreeModel.InsertPosition.AS_CHILD,
							documentModelRef: rowActivation.documentModelRef
						}
					});
					return;
			}
