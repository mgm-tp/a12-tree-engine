...
/** @internal */
export function* watchToggleExpansionSaga(activityId: string): SagaGenerator<void> {
	yield* takeLatest(
		(action: unknown) => {
			return (
				...
				action.payload.engineAction.payload.button.event === "event_toggle_expansion"
			);
		},
		SagaUtils.withErrorHandling(handleToggleExpansion, {
			activityId,
...
function* handleToggleExpansion(
	...
	yield* put(
		TreeEngineActions.event({
			...
			engineAction: Events.onNodeExpansionChanged({ nodeIdentifier, nodePath })
		})
	);
}
