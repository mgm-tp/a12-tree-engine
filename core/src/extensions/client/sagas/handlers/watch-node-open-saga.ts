...
/** @internal */
export function* watchNodeOpenSaga(activityId: string): SagaGenerator<void> {
	yield* takeLatest(
		(action: unknown) => {
			return (
				...
				action.payload.engineAction.payload.button.event === "event_open_node"
			);
		},
		SagaUtils.withErrorHandling(handleNodeOpen, {
			activityId,
