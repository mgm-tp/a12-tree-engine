...
import { type LoadOperation, SagaUtils } from "../saga-utils.js";

/** @internal */
export function* watchLoadAllNodesSaga(activityId: string): SagaGenerator<void> {
	yield* takeLatest(
		(action: unknown) => {
			return (
				TreeEngineActions.event.match(action) &&
				Events.onLoadAll.match(action.payload.engineAction) &&
				action.payload.activityId === activityId
			);
		},
		SagaUtils.withErrorHandling(handle, { activityId, error: TreeEngineError.PagingError() })
	);
}

function* handle(action: Action<TreeEngineActions.EventPayload<Action<Events.OnLoadAllPayload>>>): SagaGenerator<void> {
	...
	const outdatedDataHolderDescriptors = dataHolders
		.filter((dh) => dh.descriptor.source === nodeIdentifier.id)
		.map((dh) => dh.descriptor);

	let lockId: string | undefined;
	try {
		lockId = yield* call(ActivitySagas.acquireActivityLock, activityId, "TreeEngine", {});
		yield* call(SagaUtils.loadActivityData, activityId, {
			operations: [{ type: "LOAD_ALL_NODES", payload: { nodeIdentifier, nodePath } }] satisfies LoadOperation[],
			dataHolderDescriptors: outdatedDataHolderDescriptors
		});

		yield* call(SagaUtils.preloadChildNodes, activityId, nodeIdentifier.id);
	} finally {
		if (lockId) {
