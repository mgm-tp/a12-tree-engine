...
function makeSagasToRunForEachActivity(setting: TreeEngineSaga.Setting): () => SagaGenerator<void> {
	...
	return function* (): SagaGenerator<void> {
		yield* takeEvery(
			...
			function* (action: AnyAction): SagaGenerator<void> {
				...
				const { sagaRegistrations, linkCreation } = setting;
				const defaultSagasMap = createDefaultSagasMap({ linkCreation });
...
function getActivity(action: AnyAction, setting?: TreeEngineSaga.Setting): Activity | undefined {
