...
import { DataSelector, Events, Identifier, TreeEngineState, UIStateSelector } from "../../../../core/store/index.js";
...
import { createAddGrandChildDataHolders, getLoadedNodeIds } from "./handlers/watch-node-expansion-saga.js";
...
export namespace SagaUtils {
	...
	export function* saveActivity(
		...
		const { done } = yield* race({
			done: take<ReturnType<typeof ActivityActions.save.done>>((a: AnyAction) => {
				return ActivityActions.save.done.match(a) && a.payload.params.activityId === activityId;
			}),
			failed: take<AnyAction>((a: AnyAction) => {
				return ActivityActions.save.failed.match(a) && a.payload.params.activityId === activityId;
			})
		});
		if (
			done &&
	...
	export function* waitUntilActivitySavingStateIsDoneOrFailed(
		activityId: string
	): SagaGenerator<{ doneAction: AnyAction | undefined; failedAction: AnyAction | undefined }> {
	...
	export function* editLinkDocument(
		...
		const { done } = yield* race({
			done: take<Action<TreeEngineActions.EditLinkDocumentPayload.Done>>((a: AnyAction) => {
				return TreeEngineActions.editLinkDocument.done.match(a) && a.payload.activityId === activityId;
			})
		});
		if (done) {
	...
	export function* loadActivityData(
	...
	): SagaGenerator<{ done?: AnyAction; failed?: AnyAction }> {
		...
		const { done, failed } = yield* race({
			done: take((anyAction: AnyAction) => {
				return TreeEngineActions.setDataHolders.match(anyAction) && anyAction.payload.activityId === activityId;
			}),
			failed: take((anyAction: AnyAction) => {
				return (
					ActivityActions.error.match(anyAction) &&
					anyAction.payload.activityId === activityId &&
					anyAction.payload.operationType === "loading"
				);
			})
		});
	...
	export function withErrorHandling<Args extends unknown[], Action extends AnyAction>(
