...
/** @internal */
export function* watchLoadMoreNodesSaga(activityId: string): SagaGenerator<void> {
	yield* takeLatest(
		(action: AnyAction) => {
			return (
				TreeEngineActions.event.match(action) &&
				Events.onLoadMore.match(action.payload.engineAction) &&
...
function* handle(action: Action<TreeEngineActions.EventPayload<Action<Events.OnLoadMorePayload>>>) {
	...
	try {
		...
		yield* call(SagaUtils.loadActivityData, activityId, {
			operations: [{ type: "LOAD_MORE_NODES", payload: { nodeIdentifier, nodePath } }] satisfies LoadOperation[],
