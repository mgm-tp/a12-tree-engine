...
/** @internal */
export function* watchLinkDeletionSaga(activityId: string): SagaGenerator<void> {
	yield* takeLatest(
		(action: AnyAction) => {
			return (
				...
				action.payload.engineAction.payload.button.event === "event_delete_link"
			);
		},
		SagaUtils.withErrorHandling(handle, { activityId, error: TreeEngineError.DeleteLinkError() })
	);
}
...
function* handle(
	...
	logger.log("Unlink node", nodePath);
