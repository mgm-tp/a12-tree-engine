...
import { SagaUtils } from "../../client/shared.js";
...
import type { TreeEngineServerConnectorFactories } from "../index.js";
import type { TreeEngineDataLoader } from "../data-loaders/data-loader.js";
import { createA12DataServiceDataLoader } from "../data-loaders/a12-data-loader.js";
...
import type { LoadPayload, SavePayload } from "./utils.js";
		...
		*provideData(config): SagaGenerator<void> {
			...
			try {
				...
				if (error instanceof RequestValidator.RequestLimitExceededError) {
					yield* call(showLimitErrorNotification, {
						config,
						titleKey: RESOURCE_KEYS.treeEngine.error.requestLimitExceeded.title,
						messageKey: RESOURCE_KEYS.treeEngine.error.requestLimitExceeded.message,
						limit: error.maxRequests
					});
				} else if (error instanceof RequestValidator.PageSizeLimitExceededError) {
					yield* call(showLimitErrorNotification, {
						config,
						titleKey: RESOURCE_KEYS.treeEngine.error.pageSizeLimitExceeded.title,
						messageKey: RESOURCE_KEYS.treeEngine.error.pageSizeLimitExceeded.message,
						limit: error.pageSize
					});
				} else {
					throw error;
				}
...
function* showLimitErrorNotification(params: {
	config: DataProvider.ProvideDataConfig;
	titleKey: string;
	messageKey: string;
	limit: number;
}): SagaGenerator<void> {
	const { config, titleKey, messageKey, limit } = params;
	const { activityId } = config;
	...
	yield* put(
		NotificationActions.add({
			activityId,
			severity: "error",
			title: LocalizableFactory.createResourceLocalizable(titleKey),
			message: LocalizableFactory.createResourceLocalizable(messageKey, {
				limit: { type: "plain", value: String(limit) }
			})
		})
	);
