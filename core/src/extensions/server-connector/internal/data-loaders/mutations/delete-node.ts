...
export function* toRequest(params: {
	mutation: TreeEngineOperation.DeleteNode;
	activityId: string;
	requestSelectorMap: RequestSelectorMap;
}): SagaGenerator<SupportedRequest[]> {
	const { nodeIdentifier } = params.mutation.payload;
	const locale = yield* select(LocaleSelectors.locale());
	const request = yield* select(
		params.requestSelectorMap.deleteDocument({
			activityId: params.activityId,
			id: RequestId.createForDeleteNode(nodeIdentifier),
			params: {
				docRef: nodeIdentifier.id,
...
}): TreeEngineOperation.DeleteNodeDone {
