...
export function* transformQueryResponse(params: {
	...
	const { queryAndRequest, responses, activityId, documentProcessors } = params;
	...
	if (DataOperation.ListQuery.isAssignableFrom(query) && JsonRpc.Request.Query.isAssignableFrom(request)) {
		...
		return { id: query.id, entries, fullSize: response.result.fullSize } satisfies DataOperation.ListQueryResult;
	}
