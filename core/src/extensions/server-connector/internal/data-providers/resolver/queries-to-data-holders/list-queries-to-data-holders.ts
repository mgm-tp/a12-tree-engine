...
import { LinkDescriptorUtils } from "../../../../../../core/models/index.js";
import {
	Identifier,
	ModelSelector,
	type ModelsState,
...
/** @internal */
export interface DataHolderBySource {
	source: string;
	dataHolders: TreeEngineDataHolder[];
	currentSize?: number;
	expectedSize?: number;
...
type GroupDataHoldersBySourceReturnType = {
	childNodesDataHoldersBySources: DataHolderBySource[];
	rootNodesDataHolders: TreeEngineDataHolder[];
	parentNodesDataHolders: TreeEngineDataHolder[];
};
/** @internal */
export function groupDataHoldersBySource(
	dataHolders: TreeEngineDataHolder[],
	defaultPageSize?: number
): GroupDataHoldersBySourceReturnType {
	const childNodesDataHoldersBySources: DataHolderBySource[] = [];
	const rootNodesDataHolders: TreeEngineDataHolder[] = [];
	const parentNodesDataHolders: TreeEngineDataHolder[] = [];
	for (const dataHolder of dataHolders) {
		if (
			...
			rootNodesDataHolders.push(dataHolder);
		} else if (TreeEngineDataHolder.Descriptor.ParentNodes.isAssignableFrom(dataHolder.descriptor)) {
			parentNodesDataHolders.push(dataHolder);
		} else if (TreeEngineDataHolder.Descriptor.ChildNodes.isAssignableFrom(dataHolder.descriptor)) {
			const { currentSize, expectedSize } = PaginationUtils.getSize(dataHolder);
			const existed = childNodesDataHoldersBySources.find(({ source }) => source === dataHolder.descriptor.source);
			if (existed) {
				existed.dataHolders.push(dataHolder);
				if (currentSize) {
					existed.currentSize = (existed.currentSize ?? 0) + currentSize;
				}
				if (expectedSize) {
					existed.expectedSize = (existed.expectedSize ?? 0) + expectedSize;
				}
			} else {
				childNodesDataHoldersBySources.push({
					source: dataHolder.descriptor.source,
			...
			throw TreeEngineError.TypeError("TreeEngine.DataHolder", { actual: dataHolder });
		}
	}
	for (const dataHoldersBySource of childNodesDataHoldersBySources) {
		if (dataHoldersBySource.currentSize === 0) {
			dataHoldersBySource.expectedSize = defaultPageSize;
		}
	}
	return { childNodesDataHoldersBySources, rootNodesDataHolders, parentNodesDataHolders };
}
