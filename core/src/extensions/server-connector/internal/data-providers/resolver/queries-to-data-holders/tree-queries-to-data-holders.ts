					...
					entities: [
						{
							...
							modelName: Identifier.from(descriptor.source).type,
							docRef: descriptor.source
						},
						{
							role: childEntity.role,
							modelName: Identifier.from(entry.docRef).type,
							docRef: entry.docRef
						}
					]
				}
	...
	const link: TreeEngineState.Link = {
		...
		linkRef: {
			...
			linkDescriptor: {
				...
				entities: [
					{
						role: linkEntry.sourceRole,
						modelName: Identifier.from(linkEntry.sourceDocRef).type,
						docRef: linkEntry.sourceDocRef
					},
					{
						role: linkEntry.targetRole,
						modelName: Identifier.from(linkEntry.targetDocRef).type,
						docRef: linkEntry.targetDocRef
					}
				],
				linkDocumentDocRef: linkDocumentEntry?.targetDocRef
			}
		},
