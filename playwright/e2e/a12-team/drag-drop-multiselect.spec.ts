...
test.describe("drag and drop with multiselect", () => {
	...
	test("should enable drag and drop if only one node is selected", async ({ page }) => {
		...
		for (let i = 0; i < 12; i++) {
			...
			await expect(node).not.toContainClass("tree-table__node--draggable");
		}
		...
		await commands.clickCheckbox(commands.getRow("Jane"));

		// Test that multiple selection disables dragging
		await expect(page.locator(Selector.COUNTER)).toContainText("2");
