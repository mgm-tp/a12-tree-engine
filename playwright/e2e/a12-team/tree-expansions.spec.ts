...
test.describe("expand/collapse whole tree subtree", () => {
	...
	test("expand/collapse the whole tree", async ({ page }) => {
		...
		await expect(rows.nth(0)).toContainText("A12");
		await commands.assertExpanded("A12", false);
	...
	test("expand/collapse sub tree", async () => {
		await utils.visitA12Team();

		// Make team UP become sub team of Engines
		await commands.dragDrop("UP", "Engines", "asChild");
		await commands.collapseNode(undefined, "Engines");
