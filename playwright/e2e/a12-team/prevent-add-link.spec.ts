...
test.describe("Prevent link added from tree engine", () => {
	...
	test("should not link the new document to A12", async ({ page }) => {
		...
		await page.locator(Selector.LIST_ITEM).getByText("DomainTeam").click();
		await commands.waitUntilLoaded();
		await page.locator("#a12-TeamName-field_c9ad3").fill("Engines");
		await page.getByRole("button", { name: "Save" }).click();

		await expect(commands.getRow("Engines")).toBeVisible();
		await commands.assertLevel("Engines", 0);
		...
		await page
			...
			.click();
		await commands.waitUntilLoaded();
		await page.locator("#a12-FirstName-F3").fill("Alpha");
		await page.getByRole("button", { name: "Save" }).click();

		await expect(page.getByText("Link person with team")).toHaveCount(0);
		await expect(commands.getRow("Alpha")).toHaveCount(0);
	});
});
