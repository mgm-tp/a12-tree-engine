...
test.describe("with relationship engine", () => {
	...
	test.describe("when adding link by standalone relationship engine", () => {
		...
		test("should update node counter and list properly", async ({ page }) => {
			...
			const availableProductsBox = getDualPaneContentBoxByTitle(page, "Products");
			await availableProductsBox
				.locator(Selector.BODY_ROW)
				.filter({ hasText: "ProdC0C0C0P0Level3" })
			...
			await candidateCategoriesBox
				.locator(Selector.BODY_ROW)
				.filter({ hasText: "CatC0C0C5Level2" })
				.locator(Selector.BUTTON, { hasText: "add" })
				.click();
			await page.getByRole("button", { name: "Submit" }).click();
			await commands.waitUntilLoaded();
		...
		test("should update list after deleting properly", async ({ page }) => {
			...
			await candidateCategoriesBox
				.locator(Selector.BODY_ROW)
				.filter({ hasText: "CatC0C1C5Level2" })
				.locator(Selector.BUTTON, { hasText: "add" })
				.click();
			await candidateCategoriesBox
				.locator(Selector.BODY_ROW)
				.filter({ hasText: "CatC0C1C4Level2" })
				.locator(Selector.BUTTON, { hasText: "add" })
				.click();
			await page.getByRole("button", { name: "Submit" }).click();
			await commands.waitUntilLoaded();
