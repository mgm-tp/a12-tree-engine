...
test.describe.skip("scroll to node - twin trees", () => {
	...
	test("should scroll to target when 'reveal target' is clicked", async ({ page }) => {
		...
		await commands.openContextMenu(commands.getRow("Gender"));
		...
		await expect(commands.getRow("Gender")).not.toBeInViewport();

		// Click on reveal-target button
		await commands.openContextMenu(commands.getRows().first());
		await page.locator(Selector.POPUP_MENU).getByText("Reveal target node").click();
		await expect(commands.getRow("Gender")).toBeVisible();
	});
});
