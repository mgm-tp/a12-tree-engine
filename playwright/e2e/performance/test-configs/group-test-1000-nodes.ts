...
import type { GroupTestConfig } from "../types.js";
import { PlaywrightUtils } from "../utils.js";

export const groupTest1000Nodes: GroupTestConfig = {
	id: "group-test-1000-nodes",
	setup: async () => {
		await generateNodes({
			categories: [1, 9],
			products: 100
	...
	singleTestConfigs: [
		...
		{
			id: "collapse.1000-nodes",
			actions: async (page: Page) => {
				const commands = new PlaywrightCommands(page);
				await commands.collapseNode(undefined, "CatC0Level0");
